          ...
          <div class="col-12">
            <div class="card">
                <div class="card-header clearfix">
                  <a href={{ route('admin.events.new') }} class="btn btn-success float-right">Əlavə et</a>
                </div>
                <!-- /.card-header -->
...
<script>
    $(function() {
                $('#index_table').DataTable({
                    ...
                    ordering: true,
                    paging: true,
                    ajax: '{{ route('admin.events.index_data') }}',
                    columns: [{
                            data: 'image',
                            name: 'image',
                            orderable: false,
                ...
                $(document).on('click', '.delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        $.ajax({
                            url: "{{ route('admin.events.delete_data') }}",
                            method: 'GET',
                            data: {
                                id: id
                            },
                ...
                $(document).on('click', '#bulk_delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        ...
                        if (id.length > 0) {
                            $.ajax({
                                url: "{{ route('admin.events.mass_remove') }}",
                                method: 'GET',
                                data: {
                                    id: id
                                },
