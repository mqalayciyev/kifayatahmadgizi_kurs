          ...
          <div class="col-12">
            <div class="card">
                <div class="card-header clearfix">
                  <a href={{ route('admin.vacancy.new') }} class="btn btn-success float-right">Əlavə et</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body table-responsive">
                  <table id="index_table" class="table table-bordered table-hover">
                    ...
                    <tr>
                      ...
                      <th>Yaradıldı</th>
                      <th></th>
                      <th class="text-center"><a href="javascript:void(0)"><i class="fas fa-trash text-danger"></i></a></th>
                    </tr>
                    </thead>
                    <tbody>
...
<script>
    $(function() {
                $('#index_table').DataTable({
                    ...
                    ordering: true,
                    paging: true,
                    ajax: '{{ route('admin.vacancy.index_data') }}',
                    columns: [{
                            data: 'name',
                        },
                        {
                ...
                $(document).on('click', '.delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        $.ajax({
                            url: "{{ route('admin.vacancy.delete_data') }}",
                            method: 'GET',
                            data: {
                                id: id
                            },
                ...
                $(document).on('click', '#bulk_delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        ...
                        if (id.length > 0) {
                            $.ajax({
                                url: "{{ route('admin.vacancy.mass_remove') }}",
                                method: 'GET',
                                data: {
                                    id: id
                                },
