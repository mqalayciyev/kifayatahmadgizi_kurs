...
<script>
    $(function() {
                $('#index_table').DataTable({
                    ...
                    ordering: true,
                    paging: true,
                    ajax: '{{ route('admin.testscore.index_data') }}',
                    columns: [{
                            data: 'image',
                            name: 'image'
                        },
                ...
                $(document).on('click', '.delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        $.ajax({
                            url: "{{ route('admin.testscore.delete_data') }}",
                            method: 'GET',
                            data: {
                                id: id
                            },
                ...
                $(document).on('click', '.view', function() {
                    ...
                    $.ajax({
                            url: "{{ route('admin.testscore.view') }}",
                            type: 'POST',
                            data: {
                                test, user, "_token" : "{{ csrf_token() }}",
                            },
                            success: function(data) {
                ...
                $(document).on('click', '#bulk_delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        ...
                        if (id.length > 0) {
                            $.ajax({
                                url: "{{ route('admin.testscore.mass_remove') }}",
                                method: 'GET',
                                data: {
                                    id: id
                                },
