          ...
          <div class="col-12">
            <div class="card">
                <div class="card-header clearfix">
                  <a href={{ route('admin.index.new') }} class="btn btn-success float-right">Əlavə et</a>
                </div>
                <!-- /.card-header -->
...
<script>
    $(function() {
                $('#index_table').DataTable({
                    ...
                    ordering: true,
                    paging: true,
                    ajax: '{{ route('admin.index.index_data') }}',
                    columns: [{
                            data: 'name',
                            name: 'name'
                        },
                ...
                $(document).on('click', '.delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        $.ajax({
                            url: "{{ route('admin.index.delete_data') }}",
                            method: 'GET',
                            data: {
                                id: id
                            },
                ...
                $(document).on('click', '#bulk_delete', function() {
                    ...
                    if (confirm("Bu məlumatı silmək istədiyinizə əminsiniz?")) {
                        ...
                        if (id.length > 0) {
                            $.ajax({
                                url: "{{ route('admin.index.mass_remove') }}",
                                method: 'GET',
                                data: {
                                    id: id
                                },
